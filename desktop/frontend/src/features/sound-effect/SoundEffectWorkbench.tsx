...
import { AudioLines, ChevronDown, ChevronUp, Download, FolderOpen, Heart, LockKeyhole, Sparkles } from "lucide-react";
...
export function SoundEffectWorkbench(props: Props) {
  ...
  return <>
    ...
    <OptionalModuleWorkbench label="音效生成工作台">
      <OptionalModuleColumn label="音效模块状态">
        ...
        <Section title="输出规范" eyebrow="NATIVE DELIVERY">
      ...
      <OptionalModuleColumn label="音效描述与参数">
        ...
        <Section title="高级参数" eyebrow="GENERATION CONTROL" actions={<button className={styles.expandButton} onClick={() => setAdvancedOpen((value) => !value)}>{advancedOpen ? <ChevronUp size={15} /> : <ChevronDown size={15} />}</button>}>
          {advancedOpen ? <fieldset className={styles.parameterGrid} disabled={!interactive}>
          ...
          </fieldset> : <div className={styles.collapsedHint}>稳定模式 · Triton 编译关闭 · 原生 48 kHz WAV</div>}
        </Section>
      </OptionalModuleColumn>

      <OptionalModuleColumn label="音效生成与项目素材">
        <Section title="生成音效" eyebrow="SOUND RENDER" actions={<Badge tone="neutral">预览</Badge>}>
        ...
        <Section title="当前音效" eyebrow="MONITOR" actions={<Badge tone="neutral">示例预览</Badge>}>
        ...
        <Section title="当前项目音效素材库" eyebrow="PROJECT SFX LIBRARY" actions={<Badge tone="neutral">0 个</Badge>}>
          <div className={styles.assetList}>
            {["雨夜远处车辆驶过", "金属门缓慢开启", "林间风吹树叶"].map((name, index) => <article key={name}><span><AudioLines size={15} /></span><div><strong>{name}</strong><small>示例内容 · {8 + index * 3} 秒</small></div><Heart size={14} /><em>WAV</em></article>)}
          </div>
