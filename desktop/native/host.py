...
from desktop.native.windows import current_window_handle, resolve_main_window_geometry
...
def json_request(path: str, method: str = "GET", timeout: float = .6) -> dict[str, Any] | None:
    return _json_request(HOST, PORT, path, method=method, timeout=timeout)


def activate_existing_instance() -> bool:
    return _activate_existing_instance(HOST, PORT)
...
class NativeHost:
    ...
    def start_tray(self) -> None:
        try:
            ...
            menu = pystray.Menu(
                pystray.MenuItem(f"打开{BUILD_INFO.product}", lambda *_: self.command("show"), default=True),
    ...
    def _current_window_handle(target_pid: int | None = None) -> int | None:
        return current_window_handle(target_pid)
    ...
    def _resolve_main_window_geometry() -> tuple[int, int, int | None, int | None]:
        return resolve_main_window_geometry(webview, startup_log)
