# 声格 VoiceGrid 1.0 架构说明

## 进程结构

根目录的 `VoiceGrid 声格.exe` 是无控制台薄启动器：它只根据自身位置校验便携目录结构，并用项目内的 `pythonw.exe` 启动桌面宿主；不封装 Python、前端或模型。`备用启动.bat` 保留为等价的兼容和排障入口。宿主先取得 Windows 命名互斥锁，再启动独立、轻量的原生品牌启动页进程；启动页在自己的 GUI 消息循环中保持移动、最小化和退出可用。FastAPI、Uvicorn、数据库、监控与托盘随后在后台初始化。
